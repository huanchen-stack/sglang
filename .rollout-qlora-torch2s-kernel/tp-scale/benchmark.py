...
class BenchmarkConfig:
    ...
    two_stream_layout: str = "flipped"
    tp_all_reduce: bool = False
...
def make_real_measurements(
    ...
    "Torch QLoRA matmul two-stream": "Plain bf16 torch.matmul LoRA patch and Marlin base overlap; this tp-scale copy defaults to flipped layout.",
    }
    ...
    for model_config in model_configs:
        ...
        for projection in filter_projections(model_config, projection_filter):
            ...
            for token_rows_value in token_rows:
                ...
                torch_matmul_base_stream = torch.cuda.Stream(device=device)
                ...
                def qlora_torch_matmul_two_stream():
                    current_stream = torch.cuda.current_stream(device)
                    if effective.two_stream_layout == "flipped":
                        torch_matmul_base_stream.wait_stream(current_stream)
                        with torch.cuda.stream(torch_matmul_base_stream):
                            base_out = compiled_marlin_base_two_stream()
                        lora_out = compiled_torch_matmul_patch()
                        current_stream.wait_stream(torch_matmul_base_stream)
                        return base_out + lora_out
                    torch_matmul_side_stream.wait_stream(current_stream)
                    with torch.cuda.stream(torch_matmul_side_stream):
                        lora_out = compiled_torch_matmul_patch()
                ...
                if (
                    effective.tp_all_reduce
                    and dist is not None
                    and projection.name in {"o", "down"}
                ):
                    wrapped_providers = []
                    for scheme, fn in providers:
                        def with_tp_all_reduce(fn=fn, scheme=scheme):
                            out = fn()
                            torch.cuda.nvtx.range_push(
                                f"tp_all_reduce:{projection.name}:{scheme}"
                            )
                            try:
                                dist.all_reduce(out)
                            finally:
                        ...
                        wrapped_providers.append((scheme, with_tp_all_reduce))
                ...
                for scheme, fn in providers:
                    ...
                    try:
                        ...
                        if cuda_profiler_range and dist is not None:
                            dist.barrier(device_ids=[local_rank])
                        if cuda_profiler_range and rank == 0:
    ...
    payload = {
        "metadata": {
            ...
            "research_summary": [
                ...
                "This tp-scale copy defaults Torch QLoRA two-stream to flipped layout: Marlin base on the side stream, Torch LoRA patch on the current stream.",
            ],
        },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--two-stream-layout",
        choices=["flipped", "lora-side"],
        default=BenchmarkConfig.two_stream_layout,
        help="Torch QLoRA two-stream layout. flipped runs Marlin on the side stream and LoRA on the current stream.",
    )
    parser.add_argument(
        "--tp-all-reduce",
        action="store_true",
        help="Under torchrun, add NCCL all-reduce after row-parallel o/down projection outputs.",
    )
...
def main() -> None:
    ...
    config = BenchmarkConfig(
        ...
        tp_all_reduce=args.tp_all_reduce,
    )
    model_configs = (
